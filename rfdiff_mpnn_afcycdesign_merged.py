...
def zfs_to_home(path: str) -> str:
    zfs_prefix = "/zfsstore/user"
    home_prefix = "/home"

    if path.startswith(zfs_prefix):
        return path.replace(zfs_prefix, home_prefix, 1)
    return path
...
def run_afcycdesign(cfg):
    ...
    peptides = sorted(glob.glob(os.path.join(peptides_dir, "*.pdb")))
    afcycdesign_script = cfg.afcycdesign_run_command
    afcycdesign_sif = cfg.afcycdesign_sif
